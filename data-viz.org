...
Peut être utilisé pour illustrer les distances entre deux points. Cet usage met bien en évidence l'augmentation des disances. [cite:@Schwabish2021p275]
...
:data-type-y:
:END:
Affiche les points de données d'une catégorie.[cite:@Schwabish2021p204] Les valeurs peuvent être affichés par des lignes ou des cercles pleins mais les cercles sont à privilégier lorsqu'il est nécessaire de déagglomérer les données.

Il est possible d'utiliser de la trensparence pour illustrer les zones de valeurs compactes. [cite:@schwabish2021p205] Utiliser une palette de couleur divergente met en évidence la répartition des valeurs mesurées et souligner la séparation de celles ci par rapport à un axe de référence. [cite@Schabish2021p205]

Les points peuvent être légèrement décalés les uns des autres par un offset pour éviter le masquage de données par surimposition. Ce mode de représentation est appelé "essaim d'abeille" (beeswarm). [cite:@Schwabish2021p206]

# -----à creuser-----
Histopoint : Wilkinson dot plot (Leland Wilkinson) Affichage des bandes de valeurs pour chaque agrégat d'un histogramme. [cite:@Schwabish2021p209]
...
Seule la catégorie ayant sa base à 0 sera facile à comparer pour le lecteur car les autres ne partagent pas la même base. [cite:@Schwabish2021p88] Il est préférable de tracer chaque diagramme de barre dans un grapgique distinct et des les disposer en [[matrice de graphique]]. [cite:@Schwabish2021p89]
...
Ce graphique doit être limité à l'illustration d'un état donnée. Il est important de garder à l'esprit que ce graphique ne donne qu'une représentation et n'est pas adapté à l'affichage de valeurs précises. [cite:@Schwabish2021p295]

Le camembert permet d'accentuer l'appartenance d'une partie individuelle à un ensemble homogène et complet. Il illustre parfaitement des fractions simples. [cite:@Wilke2019p39] Les quantités n'ont pas leurs place dans un camembert, ce sont les fractions (taux, pourcentages) qui y sont illustrés. Ainsi, l'ensemble des valeurs constituante sont strictement égale à 100% du total. [cite:@Wilke2019p50] Il représente plus efficacement les parts dont les angles sont familiers aux lecteurs soit respectivement 90°, 180° et 240° pour les taux 25%, 50% et 75%.

Ce graphine n'est pas adapté à ces cas de figure :
- les parts sont de tailles similaires, [cite:@Sosulski2019p50]
- Il y a beaucoup de catégories à afficher pour atteindre l'objectif du message,
- il y a une comparaison entre deux graphiques à réaliser, [cite:@Schwabish2021p292]
- il y a un changement entre plusieurs états. [cite:@Wilke2019p40]

Il convient de :
- Arranger les parts par orgre de valeur croissant en commençant à '12h'. [cite:@Schwabish2021p290]
- Utiliser un espace vide entre chaque part pour les faire resortir, [cite:@Sosulski2019p50]
...
- Apposer les labels directement sur les parts au moyen d'étiquette. [cite:@Sosulski2019p50]

Les parties n'étant pas mises en avant peuvent être agrégée en une catégories "autres". Leurs informations ne servant pas à appuyer le message du graphique. [cite:@Schwabish2021p295]
...
Il n'est pas encore clairement établis si la comparaison des valeurs est réalisé par la perception des angles ou par la perception des longueurs d'arcs des différentes parties. [cite:@Schwabish2021p291]

Ce diagramme possède un avantage majeur au regard des autres modes de représentation des parties d'un ensemble, il est très populaire et ainsi accessible à usager. [cite:@Schwabish2021p294] Il est également reconnu que les formes circulaires attirent l'intéret des utilisateurs. [cite:@Schwabish2021p290]
...
:data-type-y:
:END:
Montre en simultané la valeur et la relation de catégories hétérogènes pour plusieurs systèmes. [cite:@Schwabish2021p264] Permet de valoriser la valeur. [cite:@alansmithLexiqueVisuel]

Pratique pour observer les schémas et relations entre les systèmes. [cite:@mikeyiHowChooseRight2020] Ce graphique est plus efficace pour explorer des jeux de données que pour les présenter à un audimat. [cite:@Sosulski2019p51]

Chaque axes de catégories peut avoir sa propre échelle. Cela représente mieux les valeurs de chaque catégorie mais cela implique également de bien labeliser les axes pour distinguer leurs échelles. [cite:@Schwabish2021p265]

Afficher un grand nombre de jeux de données sur un même graphe risque de le rendre illisible. [cite:@Schwabish2021p265] Dans ce cas il convient d'utiliser une palette de couleur de mise en évidence et de selectionner un ou deux jeux de données à mettre en avant.

Lors de l'exploration d'un jeu de donnée à travers une interface informatique, il convient de mettre en avant la ligne pointée par l'utilisateur et de labeliser chaque points de catégorie avec la valeur associé. [cite:@Sosulski2019p51]
...
Ce diagramme incite les utilisateurs à l'explorer plus en détail. [cite:@Schwabish2021p272]
...
Cette représentation découpe un ensemble en quantité individuelles de plusieurs catégories. Les bandes sont tracées entre chaque catégories pour représenter leurs relations. La largeur des bandes indiquent la proportion des catégories correllées les unes aux autres. (Sankey diagram?) [cite:@Wilke2019p113]
...
:data-type-y:
:END:
Utilisé pour afficher la fréquence d'une variable simple regroupée en intervale de fréquence (agrégat). [cite:@Sosulski2019p48] Il affiche ainsi une distribution statistique de donnée. Il n'y a pas d'espace entre deux barres (ratio 1:0) ce qui permet de favoriser la forme de la distribution. [cite:@alansmithLexiqueVisuel]

La nature du graphique (barre) implique un calcul d'agrégat pour chaque barre. [cite:@Wilke2019p60] La largeur de l'agrégat est à évaluer par le concepteur au regard du niveau de détail souhaité. [cite:@Wilke2019p60]

L'histogramme n'est pas adapté aux cas suivant :
- l'affichage de l'ensemble des valeurs, pour cela il convient d'utiliser une [[courbe de densité]]. [cite:@Wilke2019p68]
- s'il y a peu de valeurs dans le jeu de données, [cite:@Weissgerber2015p5] préférez l'utilisation d'une [[bande de valeur]].
- l'affichage de plusieurs distributions, [cite:@Wilke2019p65] utilisez une décomposition en [[matrice de graphique]].

L'utilisation d'un histogramme permet d'identifier des schémas de distributions tels que : [cite:@Schwabish2021p182]
...
:data-type-y:
:END:
Inventé par Playfair en 1786 [cite:@Sosulski2019p2]
...
Les lignes relient chaque point de mesure. [cite:@Wong2010] Pour de grands jeux de données, il convient d'utiliser une ligne lissée sur l'ensemble du graphique plutot qu'une ligne brisée par chaque point de donnée. [cite:@Wilke2019p42]

La base des ordonnées doit être égale ou inférieure à la plus faible valeur affichée. [cite:@Wong2010]

Il n'existe pas de rêgle stricte quand à la limite de séries affichables sur un graphique. [cite:@Schwabish2021p134] Il convient de s'intéresser à la perception du lecteur recherché par le concepteur pour arbitrer les choix : [cite:@Schwabish2021p134]
- En cas d'impression, limiter le nombre de lignes simultanées à 4 au maximum, [cite:@Wong2010]
- Si l'interraction est possible avec le graphique, il est possible de tracer toutes les lignes dans un même graphique à condition d'utiliser un système de mise en évidence, [cite:@Schwabish2021p139]
- Ces cas échouant, séparez les lignes et tracez une [[matrice de graphique]].

Ne pas utiliser ce graphique pour essayer d'illustrer une différence entre deux courbes. [cite:@Schwabish2021p139]

Les axes doivent être labelisé (obligatoirement le titre de la série, optionnellement la valeur finale) directement en fin de ligne, à droite du graphique. [cite:@Krause2024p36,@Sosulski2019p55]
...
La distinction des séries est plus efficace en utilisant des teintes différentes. Dans ce cas, il convient également d'utiliser des tons différents pour assurer le maintiens des distinctions si le graphique est imprimé en nuance de gris. [cite:@Few2012p219]
Il convient également de n'utiliser que des lignes continues pour les tracés. [cite:@Few2012p219]
L'utilisation de symboles comme éléments différenciants entre les séries n'est pas pertinent. [cite:@Few2012p219]
...
:data-type-y:
:END:
Ce graphique représente des courbes de densité séparées par un offset et est utilisé pour comparer la distribution d'une vrariable entre des catégories. [cite:@Wilke2019p88] Parfait lorsque les schémas de distributions sont bien distincts entre les groupes. [cite:@mikeyiHowChooseRight2020] Cela évite que les courbes de densités des catégories s'entrevêchent.[cite:@Schwabish2021p202]

Ce graphique se met très bien à l'échelle de grands jeux de données. [cite:@Wilke2019p90]

Il est adapté à :
- la comparaison des changements de distributions à travers le temps [cite:@Wilke2019p39] associé à un gradiant de couleur pour diriger le regard vers la distribution la plus récente, [cite:@Schwabish2021p203]
- la comparaison des évolutions de deux tendances à travers le temps. [cite:@Wilke2019p91] associé à une palette de couleur binaire.

L'axe des absysses affiche la valeur de réponse et l'axe des ordonnées affiche les catégories. [cite:@Wilke2019p88]

Il convient de classer les catégories suivant un ordre logique. [cite:@Schwabish2021p203] Voir (insérer renvois ici vers l'explication de la logique de classement de catégories)

Il est possible d'afficher les barres d'erreurs dans les lignes de crêtes. Ce mode de représentation s'appel un "demi-oeil". [cite:@Wilke2019p197]

Il est possible d'afficher les nuages de points dont l'estimation de la courbe de densité est issue. Ceux-ci sont affichés en dessous des courbes de densité. Cette forme de représentation est appelé "Nuage de pluie". (ref: Micah Allan ou Schwabish2021p212) Cette représentation offre à la fois une représentation générique et une visualisation précise des données. [cite:@Schwabis2021p213] Bien que cette représentation soit très efficace, elle n'est pas très répendue. [cite:@Schwabish2021p212] Il faut être vigilent à bien expliquer son fonctionnement lors de son utilisation dans un rapport. Dans le cas d'un nuage de pluie, il faut faire attention à ce que les points de données n'entre pas en surimposition avec les courbes de densitées tracées en dessous.
...
Origine
Est utilisé pour comparer de multiples graphiques homogènes organisées en matrices. Chaque entité affiche le graphique d'une catégorie. Les graphiques sont tous à la même échelle. [cite:@Sosulski2012p52]

Conseils : Limitez les détails dans chacun des graphique. [cite:@Sosulski2019p52] Regroupez les axes pour n'afficher qu'un seul axe Y par ligne et un seul axe X par colonne.
...
- Flécher la direction de l'évolution entre deux points si les points sont bien espacés et qu'un seul jeu de donnée est affiché, [cite:@Schabish2021p102]
- Utiliser un gradiant de couleur du plus clair vers le plus foncé.

Ce graphique sers à synthétiser des variations de valeurs, il est préférable de l'utiliser pour afficher des jeux de données dont la variation n'est pas mesurée régulièrement pour éviter d'avoir trop de points à relier et donc d'éviter de créer un graphique trop encombré. [cite:@Schwabish2021p102]
...
:data-type-y:
:END:
Représentation compacte de la valeur de variables hétérogènes pour un seul système. [cite:@alansmithLexiqueVisuel] Les radards sont utilisés pour identifier les variables ayant des valeurs similaires et les exceptions. [cite:@Sosulski2019p52]

La représentation des valeurs se fait à travers la forme globale du polygonne. Il convient de coloriser l'interrieur de celui-ci pour bien faire ressortir sa forme. [cite:@Schwabish2021p267]

Utiliser une disposition en small-multiples plutot que de surimposer plusieurs polygones de valeurs. [cite:@Schwabish2021p268]

La représentation des variables est fortement correlé à la forme du polygone plutot qu'aux valeurs des sommets. [cite:@Sosulski2019p52] De ce fait, pour améliorer la forme du polygonne et offrir une représentation avec le moins de distortions possibles, il convient d'homogénéiser les échelles des axes.
...
Les changements sont liés à une évolution temporelle et à l'association de catégories. La force de ce graphique est qu'aucun de ces deux aspect ne pévaut sur l'autre. [cite:@Schwabish2021p126]

Très utile pour représenter les flux entre des catégories ou entre des états. [cite:@Schwabish2021p126]

Il convient néanmoins de limiter le nombre de catégorie  affiché pour maintenir un bon niveau de lisibilité. [cite:@Schwabish2021p129]
...
Il est très pertinent pour illustrer les parties d'un ensemble lorsque de nombreuses sous-catégories sont à afficher. [cite:@Schwabish2021p307] En ce sens, il est préféré aux camenberts.
