...
L'accès à l'information et à sa bonne interprétation permet aux organisations d'adopter une démarche de prise de décision orienté sur les données. [cite:@sosulskiDataVisualizationMade2019]  =expliquer les enjeux inhérents + références=.
...
#+note: réalisé avec [[https://gka.github.io/palettes/#/7|s|2e004d|ffffe0,ff005e,93003a|1|1][chroma.js]]
...
Suivant la localisation du graphique, les informations à présenter lors de la lecture sont de nature différentes. [cite:@frankelavskyRightToolsJob2022]
#+caption: Contenu type d'un texte alternatif selon le médium concerné
#+name: fig:alt-txt
#+attr_latex: :placement [H]
[[./img/alt-text.pdf]]

Il est contre productif d'expliquer un jeu de couleur ou le détails de textures dans un texte alternatif. [cite:@elizabethhareWritingAltText2022]
...
https://www.labo.mathieurella.fr/?p=395

* Graphiques
:PROPERTIES:
:COLUMNS: %ITEM %chart-class %data-processing %data-table %data-cleanning %data-filter %data-unit
:END:
** Arbre de mot
:PROPERTIES:
:en-name: Treeword
:chart-class: correlation
:chart-goal: discovery
:data-nature: quantitative
:data-type: text
:DATA-TRANSFERT: count
:data-table: key-value
:data-cleanning: stopwords
:data-filter: depth
:data-unit: nil
:DATA-PROCESSING: COUNT()
:END:
Représente les liens sémantiques. Utilisé pour classifier les réponses textuelles d'une enquête.(voir Paige Jarreau) Utilisé pour illustrer l'usage des mots dans un texte.(voir Martin Wattenberg & Fernanda Viegs 2007) La taille des mots représente la fréquence de leurs usages. [cite:@jonathanschwabishQualitative2021]

** Arc     
:PROPERTIES:
:EN-NAME:  Arc
:CHART-GOAL:
:CHART-CLASS: part-to-whole
:DATA-TRANSFERT: ratio
:DATA-PROCESSING:
:END:
...
** Barre divergente
:PROPERTIES:
:EN-NAME:  Diverging bar chart
:CHART-GOAL:
:CHART-CLASS: deviation
:END:
...
:PROPERTIES:
:EN-NAME:  Diverging stacked bar chart
:CHART-GOAL:
:CHART-CLASS: deviation
:END:
...
** Barre ordonnée
:PROPERTIES:
:EN-NAME:  Ordered bar chart
:CHART-GOAL:
:CHART-CLASS: ranking
:DATA-PROCESSING: ORDER BY
:END:
...
:PROPERTIES:
:EN-NAME:  Ordered proportionnal bubble chart
:CHART-GOAL:
:CHART-CLASS: ranking
:DATA-PROCESSING: ORDER BY
:END:
...
:PROPERTIES:
:en-name: Pie chart
:CHART-GOAL: education
:CHART-CLASS: part-to-whole
:DATA-NATURE: quantitative,qualitative
:DATA-TYPE:
:DATA-PROCESSING: PERCENT_RANK(OVER())
:DATA-TABLE: key-value
:END:
...
:CHART-GOAL:
:CHART-CLASS: deviation,evolution
:END:
...
:PROPERTIES:
:EN-NAME:  Parallel coordinate
:CHART-GOAL:
:CHART-CLASS: magnitude
:END:
...
:CHART-GOAL:
:CHART-CLASS: distribution
:DATA-PROCESSING: CUME_DIST() OVER()
:END:
...
** Réseau  
:PROPERTIES:
:en-name: Network diagram
:CHART-GOAL:
:CHART-CLASS: flow
:END:
Utilisé pour montrer la force et l'interconnectivité et les relations de multiples entitées. [cite:@alansmithLexiqueVisuel] La position des nœuds n’a pas de fonction logique. Elle vise à rendre le diagramme le plus lisible possible. [cite:@mikeyiHowChooseRight2020]



Il existe 4 types de diagrammes de réseaux : [cite:@jonathanschwabishRelationship2021]
- Non dirigé et non pondéré,
- Non dirigé et pondéré,
- Dirigé et non pondéré,
- Dirigé et pondéré.
...
Bien que répendu dans les analyses marketing (suivi des processus de vente), ce graphique n'est pas réellement pertinent. Sa composition ne représente pas correcement les données et il n'est pas évident de comprendre les différences entre chaque segments.[cite:@stephenfewSillyGraphsThat2012]

Si la quantité de chaque étape d'un processus est l'information la plus importante, il est préférable d'avoir recours à un diagramme en [[barre ordonnée]].

S'il est  souhaité de visualiser les pertes quantitatives au fil d'un processus, il sera préférable d'employer un diagramme de [[Sankey]].
